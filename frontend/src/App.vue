...
const submitMode = ref<'save' | 'complete'>('complete')
...
function restoreSavedPhotos() {
  ...
  for (const task of activeTask.value.tasks) {
    editBaselinePhotoIds.value[task.feishu_record_id] = task.photos.map(photo => photo.id).sort()
...
async function submitPhotos(taskId: string, mode: 'save' | 'complete' = 'complete') {
  const editing = editingTaskIds.value.has(taskId)
  if (!activeTask.value || isSubmitting.value || (uploadedTaskIds.value.has(taskId) && !editing)) return
  if (mode === 'complete' && !isTaskComplete.value) {
    window.alert('请先完成必拍项')
    return
  }
  ...
  try {
    ...
    if (mode === 'complete') editingTaskIds.value = new Set([...editingTaskIds.value].filter(id => id !== taskId))
    ...
    <div class="phone">
      ...
      <template v-else>
        ...
        <section v-else-if="page === 'capture' && activeTask" class="screen capture-screen">
          ...
          <div v-if="currentTask && (!uploadedTaskIds.has(currentTask.feishu_record_id) || editingTaskIds.has(currentTask.feishu_record_id))" class="bottom-action">
            <small>暂存上传至云端，完成后提交任务</small>
            <div class="capture-actions">
              <button class="save-action" :disabled="isSubmitting || !currentTaskHasEdits" @click="submitPhotos(currentTask.feishu_record_id, 'save')">暂存</button>
              <button :class="{ incomplete: !isTaskComplete }" :aria-disabled="!isTaskComplete || isSubmitting" :disabled="isSubmitting" @click="submitPhotos(currentTask.feishu_record_id, 'complete')">完成</button>
            </div>
          </div>
          <div v-if="submitStatus !== 'idle'" class="upload-overlay"><div class="upload-dialog"><div v-if="submitStatus === 'uploading'" class="spinner"/><b>{{ submitStatus === 'uploading' ? '正在上传图片…' : submitStatus === 'success' ? (submitMode === 'save' ? '暂存成功' : '提交成功') : '上传失败' }}</b><p>{{ submitStatus === 'uploading' ? '请勿关闭当前页面' : submitStatus === 'success' ? (submitMode === 'save' ? '图片已上传，可在图片检索中查看；任务状态未变更' : '当前规格任务已完成') : submitError }}</p><button v-if="submitStatus === 'error'" @click="submitStatus = 'idle'">知道了</button></div></div>
        </section>
...
.capture-actions{display:grid;grid-template-columns:1fr 1fr;gap:12px}.bottom-action .save-action{background:#e8efff;color:#2862df;box-shadow:none;border:1px solid #c8d9ff}.bottom-action button.incomplete,.bottom-action button:disabled{background:#dce1e9;color:#8b95a5;box-shadow:none;border-color:transparent}
</style>
